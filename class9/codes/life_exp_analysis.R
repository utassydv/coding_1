...
df <- read_csv( paste0(my_path,'data/clean/WDI_GDP_CO2_clean.csv') )
...
ggplot( df , aes(x = gdptot, y = co2_emission)) +
  geom_point() +
  geom_smooth(method="loess")+
  labs(x = "Total GDP (2018 int. const. $, PPP )",y = "CO2 emission (tons/capita)") 

# You can change the scale for Total GDP for checking log-transformation
ggplot( df , aes(x = gdptot, y = co2_emission)) +
  geom_point() +
  geom_smooth(method="loess")+
  labs(x = "Total GDP (2018 int. const. $, PPP , ln scale )",y = "CO2 emission (tons/capita)") +
...
ggplot( df , aes(x = gdppc, y = co2_emission)) +
  geom_point() +
  geom_smooth(method="loess")+
...
ggplot( df , aes(x = gdppc, y = co2_emission ))  +
  geom_point() +
  geom_smooth(method="loess")+
  labs(x = "GDP/capita (2015 int. const. $, PPP , ln scale )",y = "CO2 emission (tons/capita)") +
  scale_x_continuous( trans = log_trans(), breaks = seq(0, 120, by = 20))+
  scale_y_continuous( trans = log_trans() )
...
df <- df %>% mutate( ln_gdppc = log( gdppc ),
                     ...
                     ln_co2_emission = log (co2_emission))
...
# First model:
reg1 <- lm_robust( ln_co2_emission ~ ln_gdppc , data = df , se_type = "HC2" )
reg1
# Summary statistics
summary( reg1 )
# Visual inspection:
ggplot( data = df, aes( x = ln_gdppc, y = ln_co2_emission ) ) + 
  geom_point( color='blue') +
  geom_smooth( method = lm , color = 'red' )

# Second and third model 
reg2 <- lm_robust( ln_co2_emission ~ ln_gdppc + ln_gdppc_sq , data = df )
summary( reg2 )
ggplot( data = df, aes( x = ln_gdppc, y = ln_co2_emission ) ) + 
  geom_point( color='blue') +
...
reg3 <- lm_robust( ln_co2_emission ~ ln_gdppc + ln_gdppc_sq + ln_gdppc_cb , data = df )
summary( reg3 )
ggplot( data = df, aes( x = ln_gdppc, y = ln_co2_emission ) ) + 
  geom_point( color='blue') +
...
reg6 <- lm_robust(ln_co2_emission ~ lspline( ln_gdppc , cutoff_ln ), data = df )
summary( reg6 )
ggplot( data = df, aes( x = ln_gdppc, y = ln_co2_emission ) ) + 
  geom_point( color='blue') +
...
reg7 <- lm_robust( ln_co2_emission ~ ln_gdppc + ln_gdppc_sq , data = df , weights = population)
summary( reg7 )
...
df$reg2_res <- df$ln_co2_emission - df$reg2_y_pred 

# Find countries with largest negative errors
df %>% top_n( -5 , reg2_res ) %>% 
      select( country , ln_co2_emission , reg2_y_pred , reg2_res )
...
linearHypothesis( reg2 , "ln_gdppc = 0")
